#include <stdlib.h>
#include <string.h>

#include "./../include/memory.h"
#include "./../include/object.h"
#include "./../include/table.h"
#include "./../include/value.h"

void initTable(Table* table)
{
    table->count = 0;
    table->capacity = 0;
    table->entries = NULL;
}

void freeTable(Table* table)
{
    FREE_ARRAY(Entry, table->entries, table->capacity);
    initTable(table);
}

// Finding entry based on Key in Hash table
// Used for both, to find exisiting entry or 
// to decide where to insert new ones
static Entry* findEntry(Entry* entries, int capacity, ObjString* key)
{
    uint32_t index = key->hash % capacity;
    Entry* tombstone = NULL;

    // Searching based on Linear probing 
    for (;;) {
        Entry* entry = &entries[index];

        if (entry->key == NULL) {
            if (IS_NIL(entry->value)) {
                // Tombstone can still be used to insert value
                // Empty Entry
                return tombstone != NULL ? tombstone : entry;
            } else {
                // Tombstone Found
                if (tombstone == NULL) {
                    tombstone = entry;
                }
            }
        } else if (entry->key == key) {
            return entry;
        }

        index = (index + 1) % capacity;
    }
}

// Allocating Array of buckets
static void adjustCapacity(Table* table, int capacity)
{
    Entry* entries = ALLOCATE(Entry, capacity);
    for (int i = 0; i < capacity; i++) {
        entries[i].key = NULL;
        entries[i].value = NIL_VAL;
    }

    // Adjusting Capacity will affect capacity which is used as MOD for index
    // SO need to rebuild hash table too when adjusting capacity
    table->count = 0;
    for (int i = 0; i < table->capacity; i++) {
        Entry* entry = &table->entries[i];
        if (entry->key == NULL) {
            continue;
        }

        // Recalculating Index for entry
        Entry* dest = findEntry(entries, capacity, entry->key);
        dest->key = entry->key;
        dest->value = entry->value;
        
        // Incrementing each time non tombstone entry is encountered
        table->count++;
    }

    // Freeing old Entry table
    FREE_ARRAY(Entry, table->entries, table->capacity);

    table->entries = entries;
    table->capacity = capacity;

}


bool tableSet(Table* table, ObjString* key, Value value)
{
    // Allocating and growing entry array
    if (table->count + 1 > table->capacity * TABLE_MAX_LOAD) {
        int capacity = GROW_CAPACITY(table->capacity);
        adjustCapacity(table, capacity);
    }


    Entry* entry = findEntry(table->entries, table->capacity, key);

    bool isNewKey = entry->key == NULL;
    if (isNewKey && IS_NIL(entry->value)) {
        table->count++;
    }

    entry->key = key;
    entry->value = value;

    return isNewKey;
}

void tableAddAll(Table* from, Table* to)
{
    for (int i = 0; i < from->capacity; i++) {
        Entry* entry = &from->entries[i];
        if (entry->key != NULL) {
            tableSet(to, entry->key, entry->value);
        }
    }
}

bool tableGet(Table* table, ObjString* key, Value* value)
{
    if (table->count == 0) {
        return false;
    }

    Entry* entry = findEntry(table->entries, table->capacity, key);
    if (entry->key == NULL) {
        return false;
    }

    *value = entry->value;
    return true;
}

bool tableDelete(Table* table, ObjString* key)
{
    // Tombstone technique will be used not to break 
    // Linear probing chain

    if (table->count == 0) {
        return false;
    }

    Entry* entry = findEntry(table->entries, table->capacity, key);
    if (entry->key == NULL) {
        return false;
    }

    // Placing tombstone in entry
    entry->key = NULL;
    entry->value = BOOL_VAL(true);

    return true;
}